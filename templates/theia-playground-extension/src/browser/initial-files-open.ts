...
*/
@injectable()
export class InitialFilesOpen implements FrontendApplicationContribution {
    ...
    async onStart(_app: FrontendApplication) {
        ...
        if (files) {
            decodeURIComponent(files).split(",").forEach(uri => this.revealFile(root.resource.resolve(uri)));
        } else {
            const uri = await this.locateReadme();
            if (uri) {
                this.revealFile(uri, true);
            }
        }
    }

    protected async locateReadme(): Promise<URI | undefined> {
